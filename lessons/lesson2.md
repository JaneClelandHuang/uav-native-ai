...
|---|---:|---|
| **Analysis** — `ANALYSIS.md` | 13% | Real questions and alternatives explored, not just a list of final decisions; key assumptions named; tradeoffs argued against quality goals (latency, complexity, freedom from deadlock, scalability). |
| **Requirements** — `REQUIREMENTS.md` | 10% | Correct EARS form; each requirement precise enough to test; covers separation, UAV progress, conflict detection and response, mission completion, starting subsequent missions, and failure/exceptional conditions. |
| **Design** — `DESIGN.md` | 16% | Clear architecture and diagram; each component's responsibility stated; new interfaces / MQTT topics documented; builds on the existing infrastructure rather than bypassing it; decisions justified; addresses how the existing architecture must evolve. |
| **Implementation** | 12% | Starts from a clean clone via `start_tests.py` with no manual steps; recognizably realizes `DESIGN.md`; commands and telemetry stay on the existing MQTT infrastructure; readable. |
| **Correctness on unseen workloads** | 14% | All flights complete; minimum separation maintained; the system keeps making progress (no deadlock) on scenarios you did not develop against. |
| **Performance on unseen workloads** | 8% | Among correct solutions: total workload time, time UAVs spend unnecessarily waiting, responsiveness when coordination is required. |
| **Validation** — `VALIDATION.md` | 5% | Quantitative evidence, not "it passed"; unexpected behavior investigated and explained. |
| **AI use reflection** — `AI_USE.md` | 2% | Honest, specific account of where Claude helped, where it didn't, and what you learned. |
| **Individual code understanding (in class)** | 20% | Your answers to individualized questions about *your own* submission (see below). |
